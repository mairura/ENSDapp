...
contract Domains is ERC721URIStorage {
    ...
    //Function will give us price of a domain based on length
    ...
    function register(string calldata name) public payable {
        //Check if name is unregistered-when mapping is initialised all entries point to the zero address
        if(domains[name] != address(0)) revert AlreadyRegistered();
        if(!valid(name)) revert InvalidName(name);
        // require(domains[name] == address(0));

        uint _price = price(name);
        //Check if enough amount was paid in the transaction
        ...
        console.log("Registering %s on the contract with tokenID %d", name, tld, newRecordId);
        ...
        _setTokenURI(newRecordId, "ipfs://QmNMF44JZTAPwrZFt8kvSSe5o1CCnpBER6DWB1BmaznBku");
    ...
    //Function to gett all domain names that have been minted
    ...
    //Function to prevent adding juncky domains
